...
abstract class TweetSet {
  ...
  def descendingByRetweet: TweetList = {
    try {
      ...
      return new Cons(mostRetweetedTweet, remove(mostRetweetedTweet).descendingByRetweet)
...
class Empty extends TweetSet {
  ...
  override def mostRetweeted: Tweet =
    throw new java.util.NoSuchElementException("Calling `mostRetweeted` on an empty set.")
  /**
   * The following methods are already implemented
   */
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  override def mostRetweeted: Tweet = {
    val leftTweet =
      if (left.nonEmpty) left.mostRetweeted
      else new Tweet("", "", -1)
    val rightTweet =
